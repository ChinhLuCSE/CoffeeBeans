...
# Run the application
dotnet run --project src/CoffeeBeans.backend.Web

# Or run with Aspire (if using)
dotnet run --project src/CoffeeBeans.backend.AspireHost
```
...
```powershell
dotnet ef database update -c AppDbContext -p src/CoffeeBeans.backend.Web -s src/CoffeeBeans.backend.Web
dotnet run --project src/CoffeeBeans.backend.Web
```
